...
const { usePanelSelection, usePanelSamples, usePanelCommands, usePanelProps } = hooks;
...
export default function ArtSearchComparisonPanel() {
  ...
  const chooseExample = React.useCallback(
    async (exampleId) => {
      ...
      if (item?.queryId) {
        await commands.setSelection([item.queryId], {
          persist: false,
          source: "art-query-gallery",
          clearLasso: false,
        });
      }
    },
  ...
  const clearSelection = async () => {
    setPanelError(null);
    if (commands.clearQueryContext) await commands.clearQueryContext({ persist: true });
    if (commands.setLabelFilter) commands.setLabelFilter(null);
  };

  const toolbar = React.createElement(PanelToolbar, {
    items: [
      { id: "dataset", label: "Data", value: String(panelProps.datasetName || "Art") },
      { id: "samples", label: "Items", value: String(samplesState.totalSamples ?? panelProps.sampleCount ?? "-") },
  ...
  return React.createElement(
    Panel,
    { title: "Artwork Search Readout", toolbar },
    React.createElement(
      "div",
      {
        ref: contentRef,
