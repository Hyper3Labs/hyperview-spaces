...
A **Shared View** is a portable, read-only HyperView workspace. It preserves the
...
uv run hyperview export <workspace-id> \
  --out hyperview-spaces/shared-views/<slug>
```

`../shared-views.registry.json` maps every reviewed Shared View to its canonical
...
workspace state. Use a **Shared View** to publish and collaborate around prepared
