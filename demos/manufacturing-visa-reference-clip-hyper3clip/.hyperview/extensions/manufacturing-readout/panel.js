...
export default function ManufacturingPanel() {
  ...
  const activeModelKey = typeof state.activeModelKey === "string" ? state.activeModelKey : null;
  const activeSampleId = typeof state.activeSampleId === "string" ? state.activeSampleId : null;
  const statusChoice = choiceFromSimilarity(
    activeSampleId
      ? {
          anchor_sample_id: activeSampleId,
          source: `manufacturing-demo:${activeModelKey || ""}`,
        }
      : null,
    examples,
    models,
  ...
  const onSelectQuery = React.useCallback(
    async (sampleId, model) => {
      ...
      try {
        ...
        await patchState({ activeModelKey: model.key, activeSampleId: sampleId });
        await focusPanel("manufacturing-neighbors");
      } catch (error) {
    ...
    [examples, focusPanel, patchState, runCommand, setActiveLayout],
  );
